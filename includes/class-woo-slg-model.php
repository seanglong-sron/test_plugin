	...
	public function woo_slg_add_wp_user( $criteria ) {
		...
		if ( $this->woo_slg_check_user_exists($criteria) == false ) {
			...
			if( !empty( $wp_id ) ) { //if user is created then update some data
				...
				$send_email_to = '';
				if( !empty($woo_slg_options['woo_slg_enable_notification']) && $woo_slg_options['woo_slg_enable_notification'] == 'yes'
					&& !empty($woo_slg_options['woo_slg_send_new_account_email_to_admin']) && $woo_slg_options['woo_slg_send_new_account_email_to_admin'] == 'yes' ) {
				...
				if(!empty($woo_slg_options['woo_slg_email_notification_type'])) {
					...
					if ( $woo_slg_options['woo_slg_email_notification_type'] == 'woocommerce' && class_exists( 'Woocommerce' ) 
						&& !empty($woo_slg_options['woo_slg_enable_notification']) && $woo_slg_options['woo_slg_enable_notification'] == 'yes' ) {
			...
			if( !empty($woo_slg_social_type) && !empty($woo_slg_data) ) {
				...
				if( !empty($woo_slg_options['woo_slg_enable_gp_avatar']) && $woo_slg_options['woo_slg_enable_gp_avatar'] == "yes" ) {					
					...
					if( $woo_slg_social_type == 'googleplus' ) {						
						...
						} elseif ( $woo_slg_data['image']['url'] && !empty( $woo_slg_data['image']['url'] ) ) { // Added for backward compitibility
