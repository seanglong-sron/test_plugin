...
class Woo_Slg_Login_Buttons extends WP_Widget {
	...
	function __construct() {	
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'woo-slg-login-buttons', 'description' => __( 'A social login widget.', 'wooslg' ) );

		/* Create the widget. */	
		parent::__construct( 'woo-slg-login-buttons', __( 'WooCommerce - Social Login', 'wooslg' ), $widget_ops );
	}
	...
	function widget( $args, $instance ) {		
		...
		if( !is_user_logged_in() && woo_slg_check_social_enable() ) {			
			...
			$_SESSION['woo_slg_stcd_redirect_url_widget'] = $defaulturl;	
			...
			if( $expand_collapse_enable ) {
				...
				echo '<p class="woo-slg-info">'. __($title, 'wooslg'). 
	    	...
	    	echo '<div class="woo-slg-social-container woo-slg-widget-content'. $expand_collapse_class .'">';
	...
	function form( $instance ) {
		...
		$login_heading = isset( $woo_slg_options['woo_slg_login_heading'] ) ? $woo_slg_options['woo_slg_login_heading'] : 'Prefer to Login with Social Media';

		$defaults = array( 'title' => __($login_heading, 'wooslg'), 'expand_collapse' => '' );
		...
		<p>			
			<label for="<?php echo $this->get_field_id( 'title' ); ?>"><?php _e( 'Title:', 'wooslg'); ?></label> 
			<input class="widefat" id="<?php echo $this->get_field_id( 'title' ); ?>" name="<?php echo $this->get_field_name( 'title' ); ?>" type="text" value="<?php echo $instance['title']; ?>" />
		</p>

		<p>	
			<label for="<?php echo $this->get_field_id( 'expand_collapse' ); ?>"><?php _e( 'Expand/Collapse Social Login Buttons:', 'wooslg'); ?></label><br />
			<select id="<?php echo $this->get_field_id( 'expand_collapse' ); ?>" name="<?php echo $this->get_field_name( 'expand_collapse' ); ?>">
				<option value="" <?php echo selected( $selected, '' );?>><?php _e('None','wooslg');?></option>
				<option value="collapse" <?php echo selected( $selected, 'collapse' );?>><?php _e('Collapse','wooslg');?></option>
				<option value="expand" <?php echo selected( $selected, 'expand' );?>><?php _e('Expand','wooslg');?></option>
			</select>
		</p>
